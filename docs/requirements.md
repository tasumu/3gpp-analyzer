...
| ID | 機能 | 詳細 |
|----|------|------|
| P1-01 | FTP同期 | メタデータ先行同期、遅延ダウンロード |
| P1-02 | ファイル正規化 | doc→docx変換 |
| P1-03 | 構造抽出・チャンク化 | 見出しベース分割、メタデータ付与 |
| P1-04 | ベクトル化 | embedding生成、インデックス作成 |
| P1-05 | Evidence取得IF | RAG抽象化境界 |
| P1-06 | 文書管理UI | 一覧表示、ステータス可視化、手動トリガー |
...
| ID | 機能 | 詳細 |
|----|------|------|
| P2-01 | `analyze_single()` | 単体寄書の分析（要点、変更点、論点） |
| P2-02 | `compare_docs()` | 複数寄書の比較分析 |
| P2-03 | `generate_review_sheet()` | レビューシート生成 |
| P2-04 | 分析結果の永続化 | Firestoreに保存、再分析コスト削減 |
| P2-05 | 分析UI | 分析実行、結果表示、ダウンロード |
...
| ID | 機能 | 詳細 |
|----|------|------|
| P3-01 | 会話エージェント | Phase 2機能をツールとして呼び出し |
| P3-02 | `summarize_meeting()` | 会合全寄書のサマライズ（個別＋全体レポート） |
| P3-03 | `classify_by_topic()` | トピック別自動分類 |
| P3-04 | `detect_conflicts()` | 競合・対立の検出 |
| P3-05 | Q&A | RAGを利用した自由な質問応答 |
| P3-06 | 会合レポート生成 | 全体俯瞰、注目寄書、論点整理 |
